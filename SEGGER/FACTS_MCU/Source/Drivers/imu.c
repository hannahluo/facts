...
bool bno055_setup(struct bno055_t* bno055, nrf_drv_twi_t* i2c, uint8_t dev_addr) {
    ...
    //NRF_LOG_INFO("err4 = %d", err);

    // Reset
    // err += bno055_set_sys_rst(BNO055_SYS_RST_MSK);
    //uint8_t rst_msk = BNO055_SYS_RST_MSK;
    //err += BNO055_I2C_bus_write(i2c, dev_addr, BNO055_SYS_RST_REG, &rst_msk, 1);
    //NRF_LOG_INFO("err5 = %d", err);
    ...
    NRF_LOG_INFO("err7 = %d", err);

    //uint8_t clear_reg = BNO055_INIT_VALUE;
    //err += BNO055_I2C_bus_write(i2c, dev_addr, BNO055_SYS_TRIGGER_ADDR, &clear_reg, 1);
    //NRF_LOG_INFO("err8 = %d", err);

    uint8_t use_xtal = BNO055_CLK_SRC_MSK;
